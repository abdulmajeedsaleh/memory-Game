...
function reset(){
    ...
    seconds=0;
    stars = 3;
    document.getElementsByClassName('stars')[0].children[0].style.display = 'default'; 
    document.getElementsByClassName('stars')[0].children[1].style.display = 'default'; 
    for (let card of orgCards) {
        card.classList.remove('open','show','match');
    }
    for (let card of lockedCard) {
        card.remove();
    }
    for (let card of openCard) {
        card.remove();
    }
    shuffleCards();
}
